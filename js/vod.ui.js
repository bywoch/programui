$(document).ready(function () {
	...
	function DropDown(el) {
		// 객체 속성 초기화
		this.select = el; // 전체 선택 상자 요소
		this.placeholder = this.select.children('p'); // 선택한 옵션을 나타내는 요소
		this.opts = this.select.find('ul.dropdown > li'); // 드롭다운 내 옵션들
		this.val = ''; // 선택한 옵션의 값
		var obj = this;

		// 선택한 옵션 요소를 클릭했을 때 동작 설정
		...
		obj.opts.on('click', function () {
			...
			obj.val = opt.text(); // 선택한 옵션의 텍스트 값 저장
			obj.index = opt.index(); // 선택한 옵션의 인덱스 저장
			obj.placeholder.text(obj.val); // 선택한 옵션의 텍스트를 플레이스홀더에 표시
			obj.select.find('.dropdown').hide(); // 옵션 선택 후 드롭다운 숨김
